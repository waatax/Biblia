...
def build_items(posts, books):
    ...
    for post in posts:
        ...
        by_date[day] = {
            ...
            "wd": (m.group("wd") or "").strip() or "週%s" % "一二三四五六日"[day.weekday()],
            "rawText": ref,
            "passages": [passage],
            "link": post.get("link", ""),
        }
...
def write_js(items):
    ...
    header = (
        ...
        " * 涵蓋範圍：%s（共 %d 天）\n"
        " * 站方一天發佈一天的進度，未來日期尚未公開；重跑本腳本即可補齊。\n"
        " * 抓取日期：%s\n"
        " */\n" % (YEAR, meta["coverage"], len(items), meta["fetchedAt"])
    )
...
def main():
    ...
    items, unparsed = build_items(posts, books)
    meta = write_js(items)
